...
public:
    ...
    {
        return GetType() == Exp_Operator;
    }
    bool IsVariable(void) const
    {
        return GetType() == Exp_Variable || GetType() == Exp_LocalVariable;
    }
};
...
public:
    ...
    virtual void GenerateCode(std::ostream & out, int n) const
    {
        ...
        bool parenthesis = m_arg->IsOperator() &&
            (static_pointer_cast<OperatorExpression>(m_arg)->GetOperatorPriority() < this->GetOperatorPriority());
...
public:
    ...
    ExpressionPtr GetArg1(void)
    {
        return m_arg1;
    }
    ExpressionPtr GetArg2(void)
    {
        return m_arg2;
    }
...
public:
    ...
    ExpressionPtr GetConditionExp(void) const
    {
        return m_condition;
    }
    ExpressionPtr GetTrueExp(void) const
    {
        return m_whenTrue;
    }
    ExpressionPtr GetFalseExp(void) const
    {
        return m_whenFalse;
    }
};
...
public:
    ...
    {
        bool parenthesis = m_obj->IsOperator() &&
        ...
        if (labelIndexer)
        {
            ...
            if (m_obj->GetType() == Exp_RootTable)
            {
                ...
                m_obj->GetType() == Exp_Operator &&
                static_pointer_cast<OperatorExpression>(m_obj)->GetOperatorType() == OPER_ARRAYIND)
            {
                static_pointer_cast<ArrayIndexingExpression>(m_obj)->GenerateCode(
                    out, n, labelsDelimiter, allowExplicitThis);
                out << labelsDelimiter;
            }
            else if (m_obj->IsVariable())
            {
...
public:
    ...
    virtual void GenerateCode(std::ostream & out, int n) const
    {
        ...
        out << "{" << std::endl;
        for (vector<std::pair<ExpressionPtr, ExpressionPtr>>::const_iterator i = m_Elements.begin();
             i != m_Elements.end(); ++i)
        {
            out << ::indent(n + 1);
...
// ******************************************************************************************************************************************************************************
inline void TableBaseExpression::GenerateElementCode(
    ExpressionPtr key, ExpressionPtr value, char eolChar, std::ostream & out, int n) const
{
