...
#include "BlockState.hh"
#include "Errors.hh"
#include "Expressions.hh"
#include "Formatters.hh"
#include "NutScript.hh"
...
ExpressionPtr ToTemporaryVariable(ExpressionPtr exp)
{
    if (exp->GetType() == Exp_LocalVariable)
        return ExpressionPtr(
            new VariableExpression(static_pointer_cast<LocalVariableExpression>(exp)->GetVariableName()));
    else
        return exp;
}
...
public:
    ...
    VMState(const NutFunction & parent, int stackSize) : m_Parent(parent)
    {
    ...
    void NextInstruction(void)
    {
        ...
        for (vector<NutFunction::LocalVarInfo>::const_iterator i = m_Parent.m_Locals.begin();
             i != m_Parent.m_Locals.end(); ++i)
        {
            if (i->end_op == (m_IP - 1))
            {
    ...
    void ClearPendingStatement(StatementPtr stat, ExpressionPtr usedExpression)
    {
        if (stat->GetType() == Stat_Expression)
        {
            ...
            if (pendingStatement->Equals(usedExpression))
            {
                ...
                usedExpression->GetType() == Exp_Operator &&
    ...
    bool InitVar(int pos, ExpressionPtr exp = ExpressionPtr(), bool foreachInit = false)
    {
             ...
             i != m_Parent.m_Locals.end(); ++i)
        {
            ...
            if (i->foreachLoopState != foreachInit)
                continue;

            // Variable scope range
            if (m_IP != i->start_op && (!foreachInit || m_IP < i->start_op || m_IP > i->end_op))
                continue;
    ...
    void MergeStackVariable(
        ExpressionPtr branchCondition_TrueToUseCopy, int pos, StackElement & element,
        StatementPtr AdditionalPedningStatement)
    {
        if (m_Stack[pos].expression != element.expression && element.expression)
        {
            ...
            else
            {
                ...
                std::vector<StatementPtr> pendingStatements(
                    m_Stack[pos].pendingStatements.begin(), m_Stack[pos].pendingStatements.end());
                pendingStatements.insert(
                    pendingStatements.end(), element.pendingStatements.begin(), element.pendingStatements.end());
                if (AdditionalPedningStatement)
                    pendingStatements.push_back(AdditionalPedningStatement);

                if (InitVar(pos, mergedVar))
                {
                    for (vector<StatementPtr>::iterator i = pendingStatements.begin(); i != pendingStatements.end();
                         ++i)
...
void NutFunction::DecompileStatement(VMState & state) const
{
    ...
    switch (code)
    {
        ...
        case OP_GETK:
            state.SetVar(
                arg0,
                ExpressionPtr(new ArrayIndexingExpression(
        ...
        case OP_DELETE:
        {
            ExpressionPtr derefExpr =
                ExpressionPtr(new ArrayIndexingExpression(state.GetVar(arg1), state.GetVar(arg2)));
            ExpressionPtr deleteExpt =
                ExpressionPtr(new UnaryOperatorExpression(OperatorExpression::OPER_DELETE, derefExpr));
        ...
        case OP_ARITH:
            state.SetVar(
                arg0, ExpressionPtr(new BinaryOperatorExpression(arg3, state.GetVar(arg2), state.GetVar(arg1))));
            break;
        ...
        case OP_BITW:
            state.SetVar(
                arg0,
                ExpressionPtr(
                    new BinaryOperatorExpression(BitWiseOpcodeNames[arg3], state.GetVar(arg2), state.GetVar(arg1))));
            break;
        ...
        case OP_APPENDARRAY:
        {
            ...
            {
                ExpressionPtr appendFunctionExp = ExpressionPtr(
                    new ArrayIndexingExpression(arrayExp, ExpressionPtr(new ConstantExpression("append"))));
        ...
        case OP_GETPARENT:
            state.SetVar(
                arg0,
                ExpressionPtr(
                    new ArrayIndexingExpression(state.GetVar(arg1), ExpressionPtr(new ConstantExpression("parent")))));
            break;

        case OP_COMPARITH:
        {
            ExpressionPtr leftArg = ExpressionPtr(
                new ArrayIndexingExpression(state.GetVar(((unsigned int)arg1) >> 16), state.GetVar(arg2)));
            ExpressionPtr opExp = ExpressionPtr(
                new BinaryOperatorExpression((arg3 << 8) | '=', leftArg, state.GetVar(0x0000ffff & arg1)));
        ...
        case OP_CMP:
            state.SetVar(
                arg0,
                ExpressionPtr(new BinaryOperatorExpression(
                    ComparisionOpcodeNames[arg3], state.GetVar(arg2), state.GetVar(arg1))));
            break;

        case OP_EXISTS:
            state.SetVar(
                arg0, ExpressionPtr(new BinaryOperatorExpression('in', state.GetVar(arg2), state.GetVar(arg1))));
            break;
        ...
        case OP_INSTANCEOF:
            state.SetVar(
                arg0,
                ExpressionPtr(new BinaryOperatorExpression(
                    OperatorExpression::OPER_INSTANCEOF, state.GetVar(arg2), state.GetVar(arg1))));
            break;
        ...
        case OP_OR:
        {
            ...
            while (state.IP() < destIp && !state.EndOfInstructions())
            {
                ...
                {
                    // Last move instruction of block - make simple move instead of variable set - that will be done
                    // later by logic operator
                    state.NextInstruction();
        ...
        case OP_YIELD:
            state.PushStatement(
                StatementPtr(new YieldStatement((arg0 == 0xff) ? ExpressionPtr() : state.GetVar(arg1))));
            break;

        case OP_RESUME:
            state.SetVar(
                arg0, ExpressionPtr(new UnaryOperatorExpression(OperatorExpression::OPER_RESUME, state.GetVar(arg1))));
            break;
        ...
        case OP_FOREACH:
        {
            ...
            // var->GetStackAddress()
            // == arg2 && var->GetEndAddress() >= state.IP() && var->GetEndAddress() < loopEndIp)
            //	   )
            //	{
        ...
        case OP_DELEGATE:
            state.SetVar(
                arg0, ExpressionPtr(new DelegateOperatorExpression(state.GetVar(arg2), state.GetVar(arg1))), true);
            break;

        case OP_CLONE:
            state.SetVar(
                arg0, ExpressionPtr(new UnaryOperatorExpression(OperatorExpression::OPER_CLONE, state.GetVar(arg1))));
            break;

        case OP_TYPEOF:
            state.SetVar(
                arg0, ExpressionPtr(new UnaryOperatorExpression(OperatorExpression::OPER_TYPEOF, state.GetVar(arg1))));
            break;
...
{
    if (arg1 > 0 && (state.IP() + arg1) < (int)m_Instructions.size() &&
        (state.IP() + arg1) <= state.m_BlockState.blockEnd)
    {
    ...
    if (arg1 > 0 && (state.IP() + arg1) <= state.m_BlockState.blockEnd)
    {
        ...
        if (pos1 <= state.m_BlockState.blockEnd && m_Instructions[pos1].op == OP_JMP && m_Instructions[pos1].arg1 > 0)
        {
            ...
            if (pos2 <= state.m_BlockState.blockEnd && m_Instructions[pos2].op == OP_JZ &&
                m_Instructions[pos2].arg1 >= 0)
            {
    ...
    if (arg1 >= 0 && (state.IP() + arg1) <= state.m_BlockState.blockEnd)
    {
        ...
        if ((arg1 > 0) && (m_Instructions[ifBlockEndIp - 1].op == OP_JMP) &&
        ...
        if (gotElseBlock)
        {
                ...
                m_Instructions[elseBlockEndIp - 1].op != OP_JMP)
            {
                ...
                if ((target1 == target2) && (target1 < m_StackSize) && state.AtStack(target1) &&
                    (state.AtStack(target1)->GetType() != Exp_LocalVariable) && stackCopy->at(target1).expression &&
                    stackCopy->at(target1).expression->GetType() != Exp_LocalVariable)
                {
...
// ***************************************************************************************************************
void NutFunction::GenerateFunctionSource(
    int n, std::ostream & out, const std::string & name, const std::vector<std::string> & defaults) const
{
    if (name != "constructor")
        out << "function ";
    out << name << '(';
